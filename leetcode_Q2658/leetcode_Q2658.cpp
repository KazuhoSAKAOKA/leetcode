// leetcode_Q2658.cpp : このファイルには 'main' 関数が含まれています。プログラム実行の開始と終了がそこで行われます。
//
...
public:
    ...
    int findMaxFish(vector<vector<int>>& grid) {
        ...
        int max_point = 0;
        map<size_t, int> point_map;
        for (int y = 0; y < H; y++) {
            for (int x = 0; x < W; x++) {
                if (grid[y][x] != 0) {
                    const auto index = y * W + x;
                    const auto root = uf.root(index);
                    point_map[root] += grid[y][x];
                    max_point = max(max_point, point_map[root]);
                }
            }
        }
        return max_point;
